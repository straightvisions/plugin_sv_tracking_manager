<?php
	 ...
	 */
	class sv_tracking_manager_settings extends sv_tracking_manager{
		public $core									= NULL;
		...
		public function __construct($core){
			$this->core									= isset($core->sv_tracking_manager) ? $core->sv_tracking_manager : $core; // loads common classes
			...
			$this->settings_default						= array(
				...
				'common'								=> array(
					'TRACKING_ID'						=> array(
						'name'							=> __('Tracking ID', 'sv_tracking_manager'),
						'type'							=> 'text',
						'placeholder'					=> '',
						'desc'							=> __('You are also able to set Analytics ID via wp-config constant <strong>SV_TRACKING_MANAGER_GOOGLE_ANALYTICS_ID</strong>.', 'sv_tracking_manager'),
						'value'							=> '',
					),'SCRIPT_URL'						=> array(
						'name'							=> __('Script URL', 'sv_tracking_manager'),
						'type'							=> 'text',
						'placeholder'					=> '',
						'desc'							=> __('You can change this Analytics Script URL to something custom, if required.', 'sv_tracking_manager'),
						'value'							=> '//www.google-analytics.com/analytics.js',
					)
				),'events'								=> array(
					'CLICK'								=> array(
						'name'							=> __('Click Events', 'sv_tracking_manager'),
						'type'							=> 'text',
						'placeholder'					=> '',
						'desc'							=> __('Insert each event in a new line. Event settings are separated via commas, e.g. <strong>elementID,eventCategory,eventLabel</strong>', 'sv_tracking_manager'),
						'value'							=> '',
					),'CLICK_JS'						=> array(
						'value'							=> '',
					)
		...
		public function set_settings(){
			if(isset($_POST[$this->name])){
				if($_POST[$this->name] == 1){
					...
					$tracking_id					= defined('SV_TRACKING_MANAGER_ANALYTICS_ID') ? SV_TRACKING_MANAGER_GOOGLE_ANALYTICS_ID : $this->settings['common']['TRACKING_ID']['value'];
		...
		public function plugin_action_links($actions, $plugin_file){
			if($this->core->basename == $plugin_file){
				$links				= array(
										'settings'				=> '<a href="admin.php?page='.$this->name.'">'.__('Settings', 'sv_tracking_manager').'</a>',
										'support'				=> '<a href="https://straightvisions.com/community/" target="_blank">'.__('Support', 'sv_tracking_manager').'</a>',
										'documentation'			=> '<a href="https://straightvisions.com/" target="_blank">'.__('Website', 'sv_tracking_manager').'</a>',
				);
