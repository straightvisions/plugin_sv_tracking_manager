...
<div id="sv_settings">
	...
	<div id="sv_thankyou">
		...
		<p><?php _e('Manage Google Analytics on your Site', 'sv_tracking_manager'); ?></p>
	</div>
	<form action="#" method="post" id="sv_global_settings">
		<h2><?php _e('Common', 'sv_tracking_manager'); ?></h2>
		<div class="sv_setting sv_setting_<?php echo $this->settings_default['common']['TRACKING_ID']['type']; ?>">
			...
			<div class="sv_setting_desc"><?php
				if(!defined('SV_TRACKING_MANAGER_GOOGLE_ANALYTICS_ID')){
					echo $this->settings_default['common']['TRACKING_ID']['desc'];
				}else{
					echo 'Constant <strong>SV_TRACKING_MANAGER_GOOGLE_ANALYTICS_ID</strong> is currently defined in wp-config.';
				}
			?></div>
			<div class="sv_setting_value"><input type="text" name="common[TRACKING_ID][value]" value="<?php echo (defined('SV_TRACKING_MANAGER_GOOGLE_ANALYTICS_ID') ? SV_TRACKING_MANAGER_GOOGLE_ANALYTICS_ID : $this->settings['common']['TRACKING_ID']['value']); ?>"<?php if(defined('SV_TRACKING_MANAGER_GOOGLE_ANALYTICS_ID')){ echo ' disabled="disabled"'; } ?> /></div>
		</div>
		...
		<div style="clear:both;"><input type="submit" value="<?php echo _e('Save Settings', 'sv_tracking_manager'); ?>" /></div>
	</form>
</div>
<?php
	}
